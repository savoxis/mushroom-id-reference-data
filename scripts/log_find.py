...
This does NOT commit or push anything -- see the repo's top-level README
and task tracking for why (a platform-level GitHub access gate on the
Claude Cowork side, independent of this script). It just gets a properly
formatted line onto disk in the repo's logs/ directory, ready for a normal
`git add / commit / push` the next time a human does that.
...
def main():
    ...
    print(json.dumps({'ok': True, 'log_file': LOG_FILE, 'total_entries': line_count}))
